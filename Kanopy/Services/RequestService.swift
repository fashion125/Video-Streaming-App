...
//  Kanopy
//
//  Created by Boris Esanu on 2/2/17.
//
//
...
class RequestService: NSObject {
    ...
    override init() {
        ...
        SWGDefaultConfiguration.sharedConfig().setDefaultHeaderValue(InjectedMap.xVersionHeaderValueIOS, forKey: "X-version")
        SWGDefaultConfiguration.sharedConfig().setDefaultHeaderValue(InjectedMap.UserAgentHeaderValueIOS, forKey: "User-Agent")
    ...
    open func checkStatusVerification(userID: String!,
        ...
        _ = self.authApi?.usersUserIdActivateStatusGet(withUserId: userID,
            ...
            completionHandler: { (response: SWGActivationStatusObject?, error: Error?) in
                ...
                if error == nil {
                    AuthService.sharedInstance.statusModel = response?.statusModel()
    ...
    open func displays(completion: @escaping (([ShelfModel], [CategoryModel], UserModel, [CategoryModel]) -> Void),
        ...
        let isRokuHorizontal = NSNumber.init(value: false)
        let isRokuMenu = NSNumber.init(value: false)
        
        let task = self.homePageApi?.displayGet(withAuthorization: self.token(),
                               homepage: isHomepage,
                               categories: isCategories, rokuHorizontalCategories: isRokuHorizontal, rokuMenuCategories: isRokuMenu) { (response:SWGDisplayResponse?, error: Error?) in
        ...
        if data != nil {
            ...
            do {
                ...
                response.parse(completion: cacheCompletion)
